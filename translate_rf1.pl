...
=head1 NAME
  ...
  translate_rf1.pl - translate a nucleotide FASTA file in first frame only.

=head1 SYNOPSIS
  
  translate_rf1.pl [-help] [-tab] <fasta-file+|stdin|->
...
=head1 DESCRIPTION
  ...
  stdin, and outputs the protein translations in the first frame only using
...
while (<$fhandle>) {
	...
	if ($wait_4_1st_entry eq 'true') {
	    ...
	    &print_1_frame($old_seq_id, $old_seq_desc, $seq, $tab_output);
	    $seq='';
	}    
        }
        elsif (!(/^\s*$/i)) {
...
#print last record
&print_1_frame($seq_id, $seq_desc, $seq, $tab_output);
...
sub print_1_frame {
        ...
        my($seq_id, $seq_desc, $dna, $tab_flag) = @_;
#    my $line_width = 60;
        my $line_width = 2000;
        my $frame;
	...
	$frame = 1;
	if ($tab_flag) {
	  print "$seq_id\t", &translate_frame($dna,$frame), "\n";
	}
